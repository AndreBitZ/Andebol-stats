...
function addButton(parent, id, text) {
    ...
    button = document.createElement('button'); button.id = id; button.type = 'button'; button.className = 'w-full bg-indigo-700 hover:bg-indigo-600 text-white font-bold py-3 px-4 rounded-lg transition flex items-center justify-center gap-2'; button.textContent = text; parent.appendChild(button); return button;
}
...
function installWelcomeLoader() {
    ...
    const legacyFile = document.getElementById('welcome-file-input-A'); const legacyName = document.getElementById('file-name-A'); const legacyOpponent = document.getElementById('welcome-team-b-name'); const legacyStart = document.getElementById('startGameBtn');
    legacyFile?.closest('div')?.parentElement?.classList.add('hidden'); legacyName?.classList.add('hidden'); legacyOpponent?.classList.add('hidden'); legacyStart?.classList.add('hidden');
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installImportUI, { once: true }); else installImportUI();
setTimeout(installImportUI, 500); setTimeout(installImportUI, 1500);
