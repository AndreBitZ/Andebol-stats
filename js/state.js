        ...
        try{
            ...
            const merge=(base,value)=>{
                ...
                if(base && typeof base==='object'){
                    const result={...base};
                    if(value && typeof value==='object' && !Array.isArray(value)) Object.keys(value).forEach(key=>{result[key]=key in base?merge(base[key],value[key]):value[key];});
                    return result;
                }
            ...
            for(const side of ['A','B']){
                const team=this.state.gameData[side]; if(!team)continue;
                if(!Array.isArray(team.players))team.players=[]; if(!Array.isArray(team.officials))team.officials=[];
                ...
                if(typeof team.officialsStats.yellow!=='number')team.officialsStats.yellow=0; if(typeof team.officialsStats.twoMin!=='number')team.officialsStats.twoMin=0; if(typeof team.officialsStats.red!=='number')team.officialsStats.red=0;
                if(typeof team.isTeamSuspended!=='boolean')team.isTeamSuspended=false; if(typeof team.teamSuspensionTimer!=='number')team.teamSuspensionTimer=0;
                team.players.forEach(player=>{
                    if(!player.sanctions||typeof player.sanctions!=='object')player.sanctions={yellow:0,twoMin:0,red:0};
                    if(typeof player.sanctions.yellow!=='number')player.sanctions.yellow=0; if(typeof player.sanctions.twoMin!=='number')player.sanctions.twoMin=0; if(typeof player.sanctions.red!=='number')player.sanctions.red=0;
                    if(typeof player.onCourt!=='boolean')player.onCourt=false; if(typeof player.isSuspended!=='boolean')player.isSuspended=false; if(typeof player.disqualified!=='boolean')player.disqualified=false; if(typeof player.suspensionTimer!=='number')player.suspensionTimer=0; if(typeof player.timeOnCourt!=='number')player.timeOnCourt=0;
                    if(!Array.isArray(player.history))player.history=[]; if(!Array.isArray(player.positiveActions))player.positiveActions=[]; if(!Array.isArray(player.negativeActions))player.negativeActions=[];
                });
            }
