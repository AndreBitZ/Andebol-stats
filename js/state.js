...
export class GameStore {
    ...
    loadFromLocalStorage(){const saved=sessionStorage.getItem('handballGameSession');if(!saved)return false;try{this.state=JSON.parse(saved);if(!this.state.gameData.A.officialsStats)this.state.gameData.A.officialsStats={yellow:0,twoMin:0,red:0};for(const side of ['A','B']){const team=this.state.gameData[side];if(!team)continue;if(!Array.isArray(team.players))team.players=[];if(!Array.isArray(team.officials))team.officials=[];if(typeof team.teamYellowCards!=='number')team.teamYellowCards=0;if(!team.officialsStats)team.officialsStats={yellow:0,twoMin:0,red:0};if(typeof team.isTeamSuspended!=='boolean')team.isTeamSuspended=false;if(typeof team.teamSuspensionTimer!=='number')team.teamSuspensionTimer=0;}if(!Array.isArray(this.state.gameData.B.players))this.state.gameData.B.players=[];if(!Array.isArray(this.state.gameData.B.officials))this.state.gameData.B.officials=[];if(!Object.prototype.hasOwnProperty.call(this.state,'selectedPlayerForAction'))this.state.selectedPlayerForAction=null;this.notify();return true;}catch(e){console.error('Erro a ler dados guardados, a reiniciar...',e);return false;}}
