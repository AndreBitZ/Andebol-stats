...
function emptyTeam() {
  return { possessions:0, goals:0, shots:0, saved:0, missed:0, post:0, blocked:0, saves:0, goals_conceded:0, turnovers:0, steals:0, interceptions:0, recoveries:0, seven_meter_won:0, attack_efficiency:null, shot_efficiency:null, goalkeeper_save_efficiency:null, average_attack_duration_seconds:null, transition_attacks:0 };
}
...
function addShot(bucket, result) {
  bucket.shots += 1;
  if (SHOT_RESULTS.includes(result)) bucket[result.toLowerCase()] += 1;
  bucket.efficiency = safeRate(bucket.goals, bucket.shots);
}
...
export function calculateShotAnalytics(state = {}, side = null) {
  ...
  const sides = side === 'A' || side === 'B' ? [side] : ['A','B'];
  const result = {};
  for (const s of sides) {
    result[s] = { by_zone:bucketMap(['1','2','3','4','5','6','7','8','9']), by_distance:bucketMap(['curta','curta/média','média','longa','7m']), by_zone_distance:{} };
  }
  for (const event of events) {
    if (event.event_type !== 'SHOT') continue;
    const s = teamFromId(state, event.team_id);
    if (!result[s]) continue;
    const zone = event.shot_zone != null ? String(event.shot_zone) : null;
    const distance = event.shot_distance != null ? String(event.shot_distance) : null;
    if (zone && result[s].by_zone[zone]) addShot(result[s].by_zone[zone], event.shot_result);
    if (distance && result[s].by_distance[distance]) addShot(result[s].by_distance[distance], event.shot_result);
    if (zone && distance) {
      const key = `${zone}::${distance}`;
      if (!result[s].by_zone_distance[key]) result[s].by_zone_distance[key] = emptyShotBucket();
      addShot(result[s].by_zone_distance[key], event.shot_result);
    }
  }
  return result;
}
...
export function calculateMatchAnalytics(state = {}) {
  ...
  const result = { A:emptyTeam(), B:emptyTeam(), total_events:events.length, updated_at_seconds:Number(state.totalSeconds)||0 };
  for (const sequence of sequences) {
    const side=sequence.possession_side; if(!result[side]) continue;
    ...
    if(Number.isFinite(duration)&&duration>=0){const old=result[side].average_attack_duration_seconds;const n=result[side].possessions;result[side].average_attack_duration_seconds=old==null?duration:((old*(n-1))+duration)/n;}
  }
  for(const event of events){
    const side=teamFromId(state,event.team_id); if(!side) continue;
  ...
  result.shots = calculateShotAnalytics(state);
  return result;
}
...
export function installAnalytics(store) {
  ...
  const rebuild=()=>{store.state.matchAnalytics=calculateMatchAnalytics(store.state);try{store.saveToSessionStorage();}catch(e){console.warn('[Analytics] sessão indisponível:',e);}};
  window.addEventListener('handball:state-updated',rebuild); rebuild();
}
