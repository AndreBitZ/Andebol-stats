...
function emptyTeam(){return{possessions:0,goals:0,shots:0,saved:0,missed:0,post:0,blocked:0,saves:0,goals_conceded:0,turnovers:0,steals:0,interceptions:0,recoveries:0,assists:0,seven_meter_won:0,attack_efficiency:null,shot_efficiency:null,goalkeeper_save_efficiency:null,average_attack_duration_seconds:null,transition_attacks:0};}
function teamFromId(state,teamId){const id=String(teamId??'');const a=[state?.teamAId,state?.gameData?.A?.teamId,state?.gameData?.A?.id].filter(x=>x!=null).map(String);const b=[state?.teamBId,state?.gameData?.B?.teamId,state?.gameData?.B?.id].filter(x=>x!=null).map(String);if(a.includes(id))return'A';if(b.includes(id))return'B';return null;}
function eventVal(event,key){return event?.[key]??event?.metadata?.[key]??event?.meta?.[key]??null;}
...
export function calculateShotAnalytics(state={},side=null){
 ...
 for(const event of events){if(event.event_type!=='SHOT')continue;const s=teamFromId(state,event.team_id);if(!result[s])continue;const zone=eventVal(event,'shot_zone')!=null?String(eventVal(event,'shot_zone')):null;const distance=eventVal(event,'shot_distance')!=null?String(eventVal(event,'shot_distance')):null;const goal=eventVal(event,'goal_zone_3x3')!=null?String(eventVal(event,'goal_zone_3x3')):(eventVal(event,'goal_location')!=null?String(eventVal(event,'goal_location')):null);if(zone&&result[s].by_zone[zone])addShot(result[s].by_zone[zone],event.shot_result);if(distance&&result[s].by_distance[distance])addShot(result[s].by_distance[distance],event.shot_result);if(goal&&result[s].by_goal[goal])addShot(result[s].by_goal[goal],event.shot_result);if(zone&&distance&&DISTANCES.includes(distance)){const key=`${zone}::${distance}`;if(!result[s].by_zone_distance[key])result[s].by_zone_distance[key]=emptyShotBucket();addShot(result[s].by_zone_distance[key],event.shot_result);}}
 return result;
}
