...
function togglePlayerOnCourt(side,id){
  ...
  if(!player.onCourt && currentCount>=7){ alert('⚠️ Uma equipa não pode ter mais de 7 jogadores em campo. Retire primeiro um jogador de campo.'); return; }
  store.update(s=>{ const p=(s.gameData?.[side]?.players||[]).find(x=>String(x.id??x.Numero)===String(id)||String(x.Numero)===String(id)); if(p) p.onCourt=!Boolean(p.onCourt); });
}
function ensurePopup(){
  ...
  popup.addEventListener('click',e=>{if(e.target===popup)closePopup();}); popup.querySelector('#bilateral-popup-close').onclick=closePopup;
  popup.querySelectorAll('[data-action]').forEach(btn=>btn.onclick=()=>{
    const sel=popup._selection;if(!sel)return; const action=btn.dataset.action;
    if(action==='shot'&&typeof window.openBilateralShot==='function'){ closePopup(); window.openBilateralShot(sel.side,sel.playerId); }
    else if((action==='positive'||action==='negative')&&typeof window.openBilateralAction==='function'){ closePopup(); window.openBilateralAction(sel.side,sel.playerId,action); }
    else if(action==='sanction'&&typeof window.openBilateralSanction==='function'){ closePopup(); window.openBilateralSanction(sel.side,sel.playerId); }
  });
  return popup;
}
...
window.openBilateralPlayerPopup=openPopup; window.closeBilateralPlayerPopup=closePopup;
...
export function renderHomeRoster(){
  ...
  const gks=players.filter(p=>String(p.Posicao||'').toUpperCase()==='GR'),field=players.filter(p=>String(p.Posicao||'').toUpperCase()!=='GR'); if(goalkeeperList)goalkeeperList.innerHTML=gks.map(playerRow).join(''); if(playerList)playerList.innerHTML=field.map(playerRow).join('');
  document.querySelectorAll('#player-list-A [data-player-popup], #goalkeeper-list-A [data-player-popup]').forEach(card=>{card.onclick=e=>{if(e.target.closest('[data-court-toggle]'))return;openPopup('A',card.dataset.player);};});
