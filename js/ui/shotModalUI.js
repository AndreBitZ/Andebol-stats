...
function renderConstruction(modal){
 ...
 for(const type of ['assist','imbalance']){
  ...
  [['Nenhum',null],...players.map(p=>['#'+p.Numero+' '+p.Nome,String(p.id??p.Numero)])].forEach(([label,id])=>{
   ...
   if(id&&String(selection?.[type==='assist'?'assistPlayerId':'imbalancePlayerId']??'')===String(id))b.classList.add('shot-zone-selected');
   b.onclick=()=>{const p=id?players.find(x=>String(x.id??x.Numero)===String(id)):null;if(type==='assist'){selection.assistPlayerId=id;selection.assistPlayerName=p?'#'+p.Numero+' '+p.Nome:null;if(id&&String(selection.imbalancePlayerId)===String(id)){selection.imbalancePlayerId=null;selection.imbalancePlayerName=null;}}else{selection.imbalancePlayerId=id;selection.imbalancePlayerName=p?'#'+p.Numero+' '+p.Nome:null;if(id&&String(selection.assistPlayerId)===String(id)){selection.assistPlayerId=null;selection.assistPlayerName=null;}}renderConstruction(modal);};box.appendChild(b);
  });
 }
}
...
function ensure(){
  ...
  modal.innerHTML=`<style>
    .shot-ui-btn{border:1px solid #4b5563;background:#374151;color:#fff;font-weight:800;border-radius:12px;min-height:58px;padding:12px 8px;transition:.12s;cursor:pointer}
    .shot-ui-btn:hover{background:#2563eb;border-color:#60a5fa}
    ...
    #shot-distance-modal{position:fixed;inset:0;z-index:10020;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.70);padding:16px}
    #shot-distance-modal .shot-distance-panel{width:100%;max-width:520px;border-radius:16px;background:#111827;border:1px solid #374151;box-shadow:0 25px 70px rgba(0,0,0,.55);padding:20px}
    #shot-distance-modal .shot-distance-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    #shot-distance-modal .shot-distance-choice{min-height:96px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
    #shot-distance-modal .shot-distance-cancel{width:100%;margin-top:12px;border:0;border-radius:12px;background:#374151;color:#fff;font-weight:800;padding:12px;cursor:pointer}
    #shot-distance-modal .shot-distance-cancel:hover{background:#4b5563}
    @media (max-width:520px){#shot-distance-modal .shot-distance-grid{grid-template-columns:1fr}}
  </style>
      ...
      <div class=\"grid lg:grid-cols-[1.25fr_.75fr] gap-5\">
        <section class=\"rounded-2xl bg-gray-800 p-4 border border-gray-700\">
          ...
          <div id=\"shot-zone-container\" class=\"shot-zone-grid\">
            <div class=\"shot-row-5\">${[1,2,3,4,5].map(z=>makeButton(`<span class=\"shot-zone-number\">${z}</span>`,{'data-zone':z},'shot-zone-btn')).join('')}</div>
            <div class=\"shot-row-3\">${[6,7,8].map(z=>makeButton(`<span class=\"shot-zone-number\">${z}</span>`,{'data-zone':z},'shot-zone-btn')).join('')}</div>
            <div class=\"shot-row-1\">${makeButton('<span class=\"shot-zone-number\">9</span>',{'data-zone':'9'},'shot-zone-btn')}</div>
          </div>
        ...
        <section class=\"space-y-5\">
          <div class=\"rounded-2xl bg-gray-800 p-4 border border-gray-700\"><h3 class=\"text-lg font-bold text-white mb-1\">Local do remate na baliza</h3><p class=\"text-xs text-gray-400 mb-3\">Baliza dividida em 9 partes iguais.</p><div id=\"shot-goal-container\" class=\"shot-goal-grid\">${Array.from({length:9},(_,i)=>i+1).map(z=>makeButton(`<span class=\"shot-goal-number\">${z}</span>`,{'data-goal-cell':z},'shot-goal-btn')).join('')}</div><div class=\"mt-3 text-xs text-gray-400\">Zona de baliza selecionada: <strong id=\"goal-cell-label\" class=\"text-white\">—</strong></div></div>
...
function selectZone(zone,modal){
  if(['2','3','4'].includes(zone)){showDistanceChoice(zone,modal);return;}
  selection.zone=zone; selection.distance=ZONES.find(z=>z.id===zone)?.distance||null; selection.isSevenMeter=false; update(modal);
}

function showDistanceChoice(zone,modal){
  const existing=document.getElementById('shot-distance-modal'); if(existing)existing.remove();
  const overlay=document.createElement('div');
  overlay.id='shot-distance-modal';
  overlay.innerHTML=`<div class=\"shot-distance-panel\"><div class=\"text-xl font-bold text-white mb-1\">Distância do remate</div><div class=\"text-sm text-gray-400 mb-5\">Zona ${zone}: escolha a categoria de distância.</div><div class=\"shot-distance-grid\"><button type=\"button\" data-distance-choice=\"curta\" class=\"shot-ui-btn shot-distance-choice\"><span class=\"block text-lg\">📍 Curta distância</span><span class=\"block text-xs text-gray-400 mt-1\">Até 9 metros</span></button><button type=\"button\" data-distance-choice=\"média\" class=\"shot-ui-btn shot-distance-choice\"><span class=\"block text-lg\">📍 Média distância</span><span class=\"block text-xs text-gray-400 mt-1\">Entre curta e 9m</span></button></div><button type=\"button\" data-distance-cancel class=\"shot-distance-cancel\">Cancelar</button></div>`;
  document.body.appendChild(overlay);
  overlay.querySelector('[data-distance-cancel]').onclick=()=>overlay.remove();
  overlay.querySelectorAll('[data-distance-choice]').forEach(btn=>btn.onclick=()=>{selection.zone=zone;selection.distance=btn.dataset.distance;selection.isSevenMeter=false;overlay.remove();update(modal);});
}
...
function update(modal){
  ...
  if(selection.goalCell){modal.querySelector(`[data-goal-cell=\"${selection.goalCell}\"]`)?.classList.add('shot-goal-selected');modal.querySelector('#goal-cell-label').textContent=selection.goalCell;}else modal.querySelector('#goal-cell-label').textContent='—';
...
function save(){
  ...
  try{
    const savedSelection={...selection}; const attribution={assist_player_id:savedSelection.assistPlayerId||null,assist_player_name:savedSelection.assistPlayerName||null,imbalance_player_id:savedSelection.imbalancePlayerId||null,imbalance_player_name:savedSelection.imbalancePlayerName||null,construction_label:(savedSelection.assistPlayerId||savedSelection.imbalancePlayerId)?'construção da jogada':null}; const p=player(savedSelection.side,savedSelection.playerId); if(!p)throw new Error('Atleta não encontrado.');
...
function open(side,id){
  ...
  selection={side,playerId:String(id),zone:null,distance:null,isSevenMeter:false,goalCell:null,result:null,assistPlayerId:null,assistPlayerName:null,imbalancePlayerId:null,imbalancePlayerName:null}; window.__pendingShotAttribution=null;
