...
function buildFilename(match) {
    const home = String(match?.homeTeamName || 'equipa-a')
        .replace(/[^a-z0-9]+/gi, '-')
        .replace(/^-|-$/g, '')
        .toLowerCase();
    const away = String(match?.awayTeamName || 'equipa-b')
        .replace(/[^a-z0-9]+/gi, '-')
        .replace(/^-|-$/g, '')
        .toLowerCase();
...
export function exportCanonicalMatch(options = {}) {
    try {
        ...
        console.info('[Canonical Match] Exportação concluída', {
...
// Expor para consola/integrações futuras sem acoplar a interface ao adapter.
